...
use core\extension\ui\scss\Block;
use core\extension\ui\scss\Colors;
use core\extension\ui\scss\formatter\Nested;
use core\extension\ui\scss\compiler\Environment;
...
use core\extension\ui\scss\Parser;
use core\extension\ui\scss\Util;
...
class Compiler {
    ...
    static protected $operatorNames = ['+' => 'add','-' => 'sub','*' => 'mul','/' => 'div','%' => 'mod','==' => 'eq','!=' => 'neq','<' => 'lt','>' => 'gt','<=' => 'lte','>=' => 'gte','<=>' => 'cmp'];
    static protected $namespaces = ['special' => '%','mixin' => '@','function' => '^',];
    ...
    protected $registeredFeatures = ['extend-selector-pseudoclass' => false,'at-error' => true,'units-level-3' => false,
    ...
    protected $formatter        = 'core\extension\ui\scss\formatter\Nested';
    protected $rootEnv;
    protected $rootBlock;
    protected $env;
    protected $scope;
    ...
    private function compileWith($with) {
        static $mapping = ['rule' => self::WITH_RULE,'media' => self::WITH_MEDIA,'supports' => self::WITH_SUPPORTS,
                           'all'  => self::WITH_ALL,];
    ...
    protected function opColorNumber($op,$left,$right) {
        ...
        return $this->opColorColor($op,$left,[Type::T_COLOR,$value,$value,$value]);
    }

    protected function opNumberColor($op,$left,$right) {
        $value = $left[1];
        return $this->opColorColor($op,[Type::T_COLOR,$value,$value,$value],$right);
    }
    ...
    protected function multiplyMedia(Environment $env = null,$childQueries = null) {
        ...
        $parentQueries = isset($env->block->queryList) ? $env->block->queryList : [[[Type::T_MEDIA_VALUE,
                                                                                     $env->block->value]]];
    ...
    private function replaceParameters($code) {
        if (preg_match_all("/\{([^\{]+?)\}/", $code, $match)) {
            return $match[1];
        }
        return [];
    }
    
    public function parseParseDotData($code) {
        $matches = $this->replaceParameters($code);
        if (count($matches) > 0) {
            ...
            foreach ($matches as $match) {
                ...
                foreach ($subMatches as $innerMatch) {
                    ...
                    if ($found === false) {
                        if (Heepp::dataKeyExists($innerMatch)) {
                            $found = true;
                            $dataValue = Heepp::data($innerMatch);
                            if (is_string($dataValue)) {
                                $code = str_replace('${'.$innerMatch.'}',$dataValue,$code);
                            } else {
                                $code = str_replace('${'.$innerMatch.'}',json_encode($dataValue),$code);
                            }
                        }
                        /* If no dotData matched the matches then leave the string as it was, but to stop the
                         * parser to parse the same ${} I will replace ${ with ^{^ and after all the parsing is done
                         * will replafe ^{^ to ${ again to leave the unmatched ${} string as they were */
                        if ($found === false) {
                            $code = str_replace('${'.$innerMatch.'}','^{^'.$innerMatch.'}',$code);
                           
                        }
                    } elseif ($subMatchesFound) {
                        $nextParams = $this->replaceParameters($code);
                        foreach ($nextParams as $paramKey => $paramValue) {
                            $code = str_replace('${'.$paramValue.'}','',$code);
                        }
                    }
                }
            }
        ...
        if (is_string($code) && (strpos($code,'${') !== false)) {
            $code = $this->parseParseDotData($code);
        }
        return $code;
    }
        ...
        foreach($this->importPaths as $dir) {
            if (is_string($dir)) {
                ...
                foreach($urls as $full) {
                    ...
                    if ($this->fileExists($file = $full.'.scss') || ($hasExtension && $this->fileExists($file = $full))) {
                        return $file;
                    }
                }
    ...
    public function throwError($msg) {
        ...
        $line = $this->sourceLine;
        $keys = array_keys($this->parsedFiles);
        $file = end($keys);
        $msg  = "$msg: line: $line, file: $file, included: ".serialize($keys);
        ...
        switch($value[0]) {
            ...
            case Type::T_KEYWORD:
                ...
                if (isset(Colors::$cssColors[$name])) {
                    ...
                    return isset($rgba[3]) ? [Type::T_COLOR,(int)$rgba[0],(int)$rgba[1],(int)$rgba[2],
                                              (int)$rgba[3]] : [Type::T_COLOR,(int)$rgba[0],(int)$rgba[1],
                                                                (int)$rgba[2]];
                }
                return null;
        }
    ...
    protected function libMix($args) {
        ...
        $w2 = 1.0 - $w1;
        $new = [Type::T_COLOR,$w1 * $first[1] + $w2 * $second[1],$w1 * $first[2] + $w2 * $second[2],
