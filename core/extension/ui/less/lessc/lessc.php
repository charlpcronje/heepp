 ...
 */
namespace core\extension\ui\less\lessc;
...
class lessc {
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		...
		// DO: need to mark the source parser	these came from this file
	...
	protected function compileImportedProps($props, $block, $out, $sourceParser, $importDir) {
		...
		// DO: this is because the importDir api is stupid
	...
	public function compileFile($fname, $outFname = null) {
	    if (!file_exists($fname)) {
            $fname = str_replace([env('project.url'),'/'],array(env('project.path'),DS),$fname);
            if (isset($outFname)) {
                $outFname = str_replace([env('project.url'),'/','..css'],array(env('project.path'),DS,'.css'),$outFname);
            }
        }
	...
	protected function newFormatter() {
		$className = lessc_formatter_lessjs::class;
...
class lessc_parser {
	...
	// these properties will suppress division unless it's inside parentheses
	...
	/**
	 * if we are in parents we can be more liberal with whitespace around
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('\core\extension\ui\less\lessc\lessc', 'preg_quote'),
	...
	public function parse($buffer) {
		...
		// DO report where the block was opened
	...
	protected function isDirective($dirname, $directives) {
		$pattern = implode("|",
			array_map(array(lessc::class,"preg_quote"), $directives));
	...
	protected function pushBlock($selectors=null, $type=null) {
		...
		$b->isVararg = false; // DO: kill me from here
		$b->tags = $selectors;
		$b->props = [];
		$b->children = [];
		$this->env = $b;
	...
	// DO: make it work for all functions, not just url
