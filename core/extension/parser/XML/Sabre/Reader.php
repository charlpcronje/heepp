<?php
namespace core\extension\parser\XML\Sabre;
/**
...
class Reader extends \XMLReader {
    ...
    function parseCurrentElement() {
        ...
        $name = str_replace('{}','',$this->getClark());
    ...
    function getDeserializerForElementName($name) {
        if (!array_key_exists($name, $this->elementMap)) {
            return array('\core\extension\parser\XML\Sabre\Element\Base','xmlDeserialize');
        }

        $deserializer = $this->elementMap[$name];
        if (is_subclass_of($deserializer, 'XmlDeserializable')) {
