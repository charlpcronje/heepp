...
//   Strongly simplified and intuitive DOM API, with respect to the DOM Heepp Level 1
//   Works with PHP >= 3
...
class VFXP_Element {
	 ...
	 * @param string $value
	 */
	function setValueXP($path, $value) {
		$arrNames = explode('/', $path, 2);
		$name = $arrNames[0];
		if (!$this->hasChildren($name)) {
			$subelement =& new EpstXpElement($name);
			$this->_subelements[] =& $subelement;
		} else {
			$subelement =& $this->getFirstChild($name);
		}
		if (!isset($arrNames[1])) {
			$subelement->_value = $value;
		} else {
			$subelement->setValueXP($arrNames[1], $value);
		}
	}
	 ...
	 * 
	 * @param string $path
	 * @return EpstXpElement
	 */
	function & getNodeXP($path) {
		$arrNames = explode('/', $path, 2);
		$name = $arrNames[0];
		$cond = '';
		$isCondArrayPos = false;
		$isCondChildValue = false;
		$arr2 = explode('[', $name);
		if (count($arr2) > 1) {
			$name = $arr2[0];
			$cond = substr($arr2[1], 0, -1);
			if (is_numeric($cond)) {
				$isCondArrayPos = true;
			} else {
				$isCondChildValue = true;
				$arr3 = explode('=', $cond);
				$childName = $arr3[0];
				$childValue = $arr3[1]; //substr($arr[1], 1, -1);
			}
		}
		if ($isCondArrayPos) {
			$subnode =& $this->_subelements[$cond - 1];
			if (!isset($arrNames[1])) {
				return $subnode;
			} else {
				return $subnode->getNodeXP($arrNames[1]);
			}
		} else {
			for ($pos = 0; $pos < count($this->_subelements); ++$pos) {
				$subnode =& $this->_subelements[$pos];
				if ($subnode->_name == $name) {

					if ($isCondChildValue) {
						$valid = false;
						foreach ($subnode->getChildren($childName) as $subsubnode) {
							if ($subsubnode->_value == $childValue) {
								// found
								$valid = true;
								break;
							}
					...
					if ($valid) {
						if (!isset($arrNames[1])) {
							// no more search, return first node found
							return $subnode;
						} else {
							$res =& $subnode->getNodeXP($arrNames[1]);
							if (!is_null($res)) {
								// found on this branch - no more search
								return $res;
							}
						}
					}
