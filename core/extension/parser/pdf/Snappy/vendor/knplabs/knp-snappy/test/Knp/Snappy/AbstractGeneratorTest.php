...
class AbstractGeneratorTest extends \PHPUnit_Framework_TestCase {
    public function testAddOption() {
        ...
        $this->assertEquals(['foo' => 'bar',
                             'baz' => 'bat'],$media->getOptions(),'->addOption() appends the option to the existing ones');
    ...
    public function testAddOptions() {
        ...
        $this->assertEquals(['foo' => 'bar',
                             'baz' => 'bat'],$media->getOptions(),'->addOptions() adds all the given options');
        $r->invokeArgs($media,[['ban' => 'bag','bal' => 'bac']]);
        $this->assertEquals(['foo' => 'bar','baz' => 'bat','ban' => 'bag',
                             'bal' => 'bac'],$media->getOptions(),'->addOptions() adds the given options to the existing ones');
    ...
    public function testSetOption() {
        ...
        $this->assertEquals(['foo' => 'abc'],$media->getOptions(),'->setOption() defines the value of an option');
    ...
    public function testSetOptions() {
        ...
        $this->assertEquals(['foo' => 'abc',
                             'baz' => 'def'],$media->getOptions(),'->setOptions() defines the values of all the specified options');
    ...
    public function testGenerate() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','prepareOutput','getCommand',
                                                                'executeCommand','checkOutput',
                                                                'checkProcessStatus',],['the_binary',[]]);
        $media->expects($this->once())->method('prepareOutput')->with($this->equalTo('the_output_file'));
        $media->expects($this->any())->method('getCommand')
            ->with($this->equalTo('the_input_file'),$this->equalTo('the_output_file'),$this->equalTo(['foo' => 'bar']))
            ->will($this->returnValue('the command'));
        $media->expects($this->once())->method('executeCommand')->with($this->equalTo('the command'));
        $media->expects($this->once())->method('checkProcessStatus')->with(null,'','','the command');
        $media->expects($this->once())->method('checkOutput')
            ->with($this->equalTo('the_output_file'),$this->equalTo('the command'));
        $media->generate('the_input_file','the_output_file',['foo' => 'bar']);
    }

    public function testGenerateFromHtml() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','generate','createTemporaryFile',],['the_binary'],'',false);
        $media->expects($this->once())->method('createTemporaryFile')
            ->with($this->equalTo('<html>foo</html>'),$this->equalTo('html'))
            ->will($this->returnValue('the_temporary_file'));
        $media->expects($this->once())->method('generate')
            ->with($this->equalTo(['the_temporary_file']),$this->equalTo('the_output_file'),$this->equalTo(['foo' => 'bar']));
    ...
    public function testGetOutput() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','getDefaultExtension','createTemporaryFile','generate','getFileContents','unlink'],[],'',false);
        $media->expects($this->any())->method('getDefaultExtension')->will($this->returnValue('ext'));
        $media->expects($this->any())->method('createTemporaryFile')->with($this->equalTo(null),$this->equalTo('ext'))
            ->will($this->returnValue('the_temporary_file'));
        $media->expects($this->once())->method('generate')
            ->with($this->equalTo('the_input_file'),$this->equalTo('the_temporary_file'),$this->equalTo(['foo' => 'bar']));
        $media->expects($this->once())->method('getFileContents')->will($this->returnValue('the file contents'));

        $media->expects($this->any())->method('unlink')->with($this->equalTo('the_temporary_file'))
            ->will($this->returnValue(true));
        $this->assertEquals('the file contents',$media->getOutput('the_input_file',['foo' => 'bar']));
    }

    public function testGetOutputFromHtml() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','getOutput',
                                                                'createTemporaryFile',],[],'',false);
        $media->expects($this->once())->method('createTemporaryFile')
            ->with($this->equalTo('<html>foo</html>'),$this->equalTo('html'))
            ->will($this->returnValue('the_temporary_file'));
        $media->expects($this->once())->method('getOutput')
            ->with($this->equalTo(['the_temporary_file']),$this->equalTo(['foo' => 'bar']))
    ...
    public function testGetOutputFromHtmlWithHtmlArray() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','getOutput','createTemporaryFile',],[],'',false);
        $media->expects($this->once())->method('createTemporaryFile')
            ->with($this->equalTo('<html>foo</html>'),$this->equalTo('html'))
            ->will($this->returnValue('the_temporary_file'));
        $media->expects($this->once())->method('getOutput')
    ...
    public function testMergeOptions() {
        ...
        $this->assertEquals(['foo' => 'ban',
                             'baz' => 'bat'],$mergedOptions,'->mergeOptions() merges an option to the instance ones and returns the result options array');
        $this->assertEquals($originalOptions,$media->getOptions(),'->mergeOptions() does NOT change the instance options');
        $mergedOptions = $r->invokeArgs($media,[['foo' => 'ban','baz' => 'bag']]);
        $this->assertEquals(['foo' => 'ban',
                             'baz' => 'bag'],$mergedOptions,'->mergeOptions() merges many options to the instance ones and returns the result options array');
    ...
    public function dataForBuildCommand() {
        ...
        return [[$theBinary,'http://the.url/','/the/path',[],
                 $theBinary.' '.escapeshellarg('http://the.url/').' '.escapeshellarg('/the/path')],
                [$theBinary,'http://the.url/','/the/path',['foo' => null,'bar' => false,'baz' => []],
                 $theBinary.' '.escapeshellarg('http://the.url/').' '.escapeshellarg('/the/path')],
                [$theBinary,'http://the.url/','/the/path',
                 ['foo' => 'foovalue','bar' => ['barvalue1','barvalue2'],'baz' => true],
                ...
                [$theBinary,'http://the.url/','/the/path',
                 ['cookie' => ['session' => 'bla','phpsess' => 12],'no-background' => '1',],
                ...
                [$theBinary,'http://the.url/','/the/path',['allow' => ['/path1','/path2'],'no-background' => '1',],
    ...
    public function testCheckOutput() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','fileExists','filesize'],[],'',false);
        $media->expects($this->once())->method('fileExists')->with($this->equalTo('the_output_file'))
            ->will($this->returnValue(true));
        $media->expects($this->once())->method('filesize')->with($this->equalTo('the_output_file'))
    ...
    public function testCheckProcessStatus() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure',],[],'',false);
    ...
    public function testItThrowsTheProperExceptionWhenFileExistsAndNotOverwritting() {
        $media = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','fileExists','isFile'],[],'',false);
        $media->expects($this->any())->method('fileExists')->will($this->returnValue(true));
        $media->expects($this->any())->method('isFile')->will($this->returnValue(true));
    ...
    public function dataForIsAssociativeArray() {
        return [[['key' => 'value'],true],[['key' => 2],true],[['key' => 'value','key2' => 'value2'],true],
                [[0 => 'value',1 => 'value2','deux' => 'value3'],true],[[0 => 'value'],false],
                [[0 => 'value',1 => 'value2',3 => 'value3'],false],
                [['0' => 'value','1' => 'value2','3' => 'value3'],false],[[],false],];
    }

    public function testCleanupEmptyTemporaryFiles() {
        $generator = $this->getMock('Knp\Snappy\AbstractGenerator',['configure','unlink',],['the_binary']);
        $generator->expects($this->once())->method('unlink');
