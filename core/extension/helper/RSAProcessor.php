...
class RSAProcessor extends \core\extension\Extension {
...
class RSAProcessor {
    ...
    public $modulus = null;
    public $key_length = "1024";

    function RSAProcessor($xmlRsakey = null, $type = null, $load = null) {
        if ($xmlRsakey == null) {
            $xmlObj = simplexml_load_file("xmlfile/RSAKey.xml");
        } else
        if ($load == RSAKeyType::XMLFile()) {
            $xmlObj = simplexml_load_file($xmlRsakey);
        } else {
            $p = xml_parser_create();
            xml_parse_into_struct($p, $xmlRsakey, $vals, $index);
            xml_parser_free($p);

            foreach ($index as $i => $a) {
                if ($i == "MODULUS") {
                    foreach ($a as $t => $v) {
                        foreach ($vals [$v] as $x => $y) {
                            if ($x == "value") {
                                $modulus = $y;
                            }
                        }
        ...
        $this->modulus = RSA::binary_to_number(base64_decode($modulus));
        if ($type == "1") {
            $this->private_key = RSA::binary_to_number(base64_decode($d));
        } else {
            $this->public_key = RSA::binary_to_number(base64_decode($exponent));
        }
        $this->key_length = strlen(base64_decode($modulus)) * 8;
    }
    ...
    function decrypt($data) {
        return RSA::rsa_decrypt(base64_decode($data), $this->private_key, $this->modulus, $this->key_length);
    }
