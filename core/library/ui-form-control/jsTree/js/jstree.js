...
(function(factory) {
	 ...
	 * @param {DomNode|jQuery|String} el the element to create the instance on, can be jQuery extended or a selector
	 ...
	 * @return {jQuery}
	 */
	$.expr[':'].jstree = $.expr.createPseudo(function() {
		return function(a) {
    ...
    $.jstree.core.prototype = {
			...
			this.element
				...
				.on('keypress.jstree', $.proxy(function(e) {
						...
						if(new RegExp('^' + chr.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '+$').test(word)) {
							...
							if(end) {
							    /* Return not nessesary because it is the last statement
							     * return;
							     */
							}
						}
		 ...
		 */
		get_node : function(obj, as_dom = true) {
			if(obj && obj.id) {
				obj = obj.id;
			}
			var dom;
		...
		load_node : function(obj, callback) {
			...
			this._load_node(obj, $.proxy(function(status) {
				...
				var dom = this.get_node(obj, true), m = this._model.data, has_children = false;
		...
		_load_nodes : function(nodes, callback, is_callback = false, force_reload = false) {
			var r = true,
		...
		_append_json_data : function(dom, data, cb, force_processing) {
			...
			var w = null,
				...
				rslt = function(rslt, worker) {
					...
					self._cnt = rslt.cnt;
					var i, m = self._model.data;
					for (i in m) {
						if (m.hasOwnProperty(i) && m[i].state && m[i].state.loading && rslt.mod[i]) {
							rslt.mod[i].state.loading = true;
						}
					}
					self._model.data = rslt.mod; // breaks the reference in load_node - careful

					if(worker) {
						var j, a = rslt.add, r = rslt.sel, s = self._data.core.selected.slice();
						m = self._model.data;
					...
					if(rslt.add.length) {
						self._data.core.selected = self._data.core.selected.concat(rslt.add);
					}
					...
					if(rslt.add.length) {
						this.trigger('changed', { 'action' : 'model', 'selected' : self._data.core.selected });
					}
					cb.call(this, true);
				};
		 ...
		 */
		redraw_node : function(node, deep = true, is_callback = false, force_render = false) {
			var obj = this.get_node(node),
				par = false,
				ind = false,
				old = false,
		 ...
		 */
		hide_node : function(obj, skip_redraw = false) {
			var t1, t2;
			if($.isArray(obj)) {
				obj = obj.slice();
				for(t1 = 0, t2 = obj.length; t1 < t2; t1++) {
	...
	$.jstree.plugins.contextmenu = function(options, parent) {
		this.bind = function() {
			...
			this.element
				...
				.on("click.jstree", ".jstree-anchor", $.proxy(function() {
		...
		$(document)
			...
			.on('dnd_move.vakata.jstree', function(e, data) {
				...
				var ins = $.jstree.reference(data.event.target),
					...
					off = false,
					rel = false,
					l, t, h, p, i, o, ok, t1, t2, op, ps, pr, ip, tm, is_copy;
 ...
 */
	...
	$.jstree.plugins.search = function(options, parent) {
		 ...
		 * @param show_only_matches_children
		 * @plugin search
	...
	$.jstree.plugins.wholerow = function(options, parent) {
		this.bind = function() {
			...
			this.element
				...
				.on("deselect_all.jstree", $.proxy(function() {
